...
During class, with everyone accessing the data at once, the server gets backed up and it takes too long to download the data. Therefore, I've placed copy of the downloaded data in the GitHub project and we'll load that using the readRDS() function. We'll talk about that function in the code below. For now, understand that when doing this yourself, you should un-comment the loadByProduct line and comment out the readRDS line.
